...
/*   Updated: 2024/07/07 21:12:17 by hitran           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	print_number(t_stack *stack, t_node **current, int i)
{
	int len;
	...
	len = ft_strlen(ft_itoa((*current)->value));
	if (i < stack->size)
	{
		write(1, "              ", 13 - len - (13 - len) / 2);
...
void	print_stack(t_pushswap *ps)
{
	int		i;
	t_node	*a_current;
	...
	i = 0;
	a_current = ps->a->head;
	ft_putendl_fd("+---Numbers---+----Index----+", 1);
	while (i < ps->a->size)
	{
		ft_putchar('|');
		print_number(ps->a, &a_current, i);
		ft_putchar('|');
